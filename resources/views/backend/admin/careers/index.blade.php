...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('careers') }}" method="GET">
                    <div class="row g-3 mb-3">
                        <div class="col-sm-4">
                            <input type="text" name="designation" class="form-control" placeholder="Search by Designation" value="{{ request('designation') }}">
                        </div>
                        <div class="col-sm-4">
                            <input type="text" name="job_location" class="form-control" placeholder="Search by Job Location" value="{{ request('job_location') }}">
                        </div>
                        <div class="col-sm-4">
                            <input type="text" name="qualification" class="form-control" placeholder="Search by Qualification" value="{{ request('qualification') }}">
                        </div>
                        <div class="col-sm-4">
                            <button type="submit" class="btn btn-primary w-100">Filter</button>
                        </div>
                        <div class="col-sm-4">
                            <a href="{{ route('careers') }}" class="btn btn-secondary w-100">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <table class="table align-middle table-nowrap" id="customerTable">
                            ...
                            <tbody class="list form-check-all">
                                ...
                                <tr>
                                    ...
                                    <td class="cv-logo">
                                        <a href="{{ url('public/uploads/careers/' . $row->image) }}" download><strong>Download CV</strong>
                                        </a>
                                    </td>
                                        ...
                                        <div class="d-flex gap-2">
                                            <div class="edit">
                                                <button class="btn btn-sm btn-success edit-item-btn" data-bs-toggle="modal" data-bs-target="#exampleModalgrid"
                                                    ...
                                                    data-job_location="{{ $row->job_location }}"
                                                    data-qualification="{{ $row->qualification }}"
                                                    data-overview="{{ $row->overview }}"
                                                    data-roles_n_responsibilities="{{ $row->roles_n_responsibilities }}"
                                                    data-experience="{{ $row->experience }}">
                                                    Edit
                                                </button>
                                            </div>
                                            <div class="remove">
                                                <a href="{{ route('career.delete', $row->id) }}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure want to delete this?')">Remove</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
    ...
    $(document).ready(function() {
        $('#careeForm').on('submit', function(e) {
            ...
            if ($('#careerId').val() === "") {
                ...
                actionUrl = "{{ route('career.update', '') }}/" + careerId;
                method = 'POST';
            }
